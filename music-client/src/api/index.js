...
const HttpManager = {
  ...
  // =======================> 博客 API
  // 返回所有博客
  getAllBlogs: (params) => post(`/blog/blog/home?currentPage=${params}`),
  // 添加博客
  getBlogDetail: (param) => post(`/blog/blog/getBlogDetail?id=${param}`, param),
  // 删除博客
  ...
  // 模糊查询博客
  getLike: (filterName, currentPage) => post(`/blog/blog/getLike?filterName=${filterName}&currentPage=${currentPage}`),
  // 根据标签名称查询博客
  getBlogByTagName: (tagName,currentPage) => post(`/blog/blog/getBlogByTagName?tagName=${tagName}&currentPage=${currentPage}`),
  ...
  // =======================> 分类 API
  // 返回所有类别
  getAllCategory: () => post(`/blog/category/getAll`),
  // 添加类别
  getBlogByCategoryName: (param,currentPage) => post(`/blog/blog/getBlogByCategoryName?categoryName=${param}&currentPage=${currentPage}`),
  // 更新类别
  updateCategory: (params) => post(`/admin/category/update`, params),
  // 删除类别
  deleteCategory: (ids) => post(`/admin/category/delete`, ids),
  // 返回的指定用户ID收藏列表
  getCategoryById: (id) => post(`/blog/category/get?Id=${id}`),
  // 返回指定歌手名的歌曲
  ...
  // 获取全部链接
  getAllLinks: () => post(`/blog/link/getAll`),
  // 添加链接
