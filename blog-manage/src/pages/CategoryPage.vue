<template>
  <div class="table">
      ...
      <el-table :data="tableData" border size="mini" style="width: 100%" height="850px" :row-style="{height:'200px'}" ref="multipleTable" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="预览图" width="500" align="center">
          <template slot-scope="scope">
            <img :src="scope.row.categoryImg" alt="" style="width: 200px;height: 150px;"/>
          </template>
        </el-table-column>
    ...
    <el-dialog title="添加分类" :visible.sync="centerDialogVisible" width="400px" center>
      <el-form :model="category"  ref="category"  label-width="80px" class="demo-ruleForm" :rules="rule">
        <el-form-item label="分类名称" prop="categoryName" size="mini" >
          <el-input v-model="category.categoryName" placeholder="分类名称"></el-input>
        </el-form-item>
        <el-form-item label="图标图床" prop="categoryImg" size="mini">
          <el-input v-model="category.categoryImg" placeholder="分类名称"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      ids: [],
      rule: {
        categoryName: [{required: true, message: '分类名称为空', tiger: 'blur'}],
        categoryImg: [{required: true, message: '分类图标为空', tiger: 'blur'}]
      }
    }
  },
  created () {
